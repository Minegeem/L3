...
typedef struct {
    ...
    unsigned int true_calloc_compteur;
    unsigned int true_realloc_compteur;
...
void _init_cell_table() { 
    ...
    _track_TABLE.true_calloc_compteur = 0;
    _track_TABLE.true_realloc_compteur = 0;
...
void _cell_bilan(void) {
    ...
    printf("<calloc>              : %d appel%c\n", _track_TABLE.true_calloc_compteur, ((_track_TABLE.true_calloc_compteur) > 1)?'s':' ');
    printf("<realloc>             : %d appel%c\n", _track_TABLE.true_realloc_compteur, ((_track_TABLE.true_realloc_compteur) > 1)?'s':' ');
...
void* _clone_malloc(char* fich, const char* fonc, int line, size_t size) {
    ...
    _track_TABLE.total_memory_alloc += size;
...
void _clone_free(char*file, const char* fonc, int line, void* adr) {
  	if(!TRACEUR) activer_traceur();
  	_track_cell* tmp = _track_TABLE.head;
    ...
    tmp->actif = 0;
    _track_TABLE.total_memory_free += tmp->size;
...
void* _clone_calloc(char* fich, const char* fonc, int line, int number, size_t size) {
    int maxsize = size * number;
  	if(!TRACEUR) activer_traceur();
    void* adr =  calloc(number, size);
    if(!adr) exit(1);
    _track_cell* adr_actuelle = _track_TABLE.head;
    while(adr_actuelle && (adr_actuelle->adresse != adr)) {adr_actuelle = adr_actuelle->next;}

    if(!(adr_actuelle)) {
        adr_actuelle = _init_cell_base(adr, maxsize, 1);
    	_add_cell_(adr_actuelle);
	}

    adr_actuelle->actif = 1;
    adr_actuelle->size = maxsize;
    _track_TABLE.true_calloc_compteur++;
    _track_TABLE.total_memory_alloc += maxsize;
    fprintf(stderr, "in file <%s> function <%s> line <0%d> – (call#0%d) calloc(%ld)->%p\n", fich, fonc, line, _track_TABLE.true_malloc_compteur, adr_actuelle->size, adr);
    return adr;
} 

void* _clone_realloc(char* fich, const char* fonc, int line, void* adresse, size_t size) {
  	if(!TRACEUR) activer_traceur();
    void* adr =  realloc(adresse, size);
    if(adr != adresse) {
        _clone_free(fich, fonc, line, adresse);
    }
    if(!adr) exit(1);
    _track_cell* adr_actuelle = _track_TABLE.head;
    while(adr_actuelle && (adr_actuelle->adresse != adr)) {
        adr_actuelle = adr_actuelle->next;
    }
    ...
    adr_actuelle->actif = 1;
    adr_actuelle->size = size;
    _track_TABLE.true_realloc_compteur++;
    _track_TABLE.total_memory_alloc += size;
    fprintf(stderr, "in file <%s> function <%s> line <0%d> – (call#0%d) realloc(%ld)->%p\n", fich, fonc, line, _track_TABLE.true_realloc_compteur, adr_actuelle->size, adr);
    return adr;
} 